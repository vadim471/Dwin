...
system::error_code basic_connection::get(const std::string& target, const header_list_t& head, const std::string& filename, const on_progress_f& on_progress, const on_response_f& on_response) {

    if (!this->connected_) {
        sys_debug_print(SYSTEM_LEVEL_WARNING, "http::ranged::basic_connection::get", "Not connected");
        return system::err::invalid_state;
    }

    if (this->busy_) {
        sys_debug_print(SYSTEM_LEVEL_WARNING, "http::ranged::basic_connection::get", "Service is busy");
        return system::err::invalid_state;
    }
    ...
    #ifdef SYSTEM_TRACE_HTTP_REQUESTS
    {
        ...
        sys_debug_print(SYSTEM_LEVEL_TRACE, "http::ranged::basic_connection::get", "%s", stream.str().c_str());
    }
    #endif // SYSTEM_TRACE_HTTP_REQUESTS

    //sys_debug_print(SYSTEM_LEVEL_DEBUG, "http::ranged::basic_connection::get", "Trying to set write timer");
...
system::error_code basic_connection::request_range(const std::string& range, const on_progress_f& on_progress, const on_response_f& on_response) {

    if (!this->connected_) {
        sys_debug_print(SYSTEM_LEVEL_WARNING, "http::ranged::basic_connection::request_range", "Not connected");
        return system::err::invalid_state;
    }
    ...
    //sys_debug_print(SYSTEM_LEVEL_DEBUG, "http::ranged::basic_connection::request_range", "Trying to set write timer");
