...
system::error_code basic_connection::connect(const std::string& host, const std::string& port, const on_result_f& on_result) {
    ...
    if (this->connected_) {
        sys_debug_print(SYSTEM_LEVEL_WARNING, "http::ranged::basic_connection::connect", "Already connected");
        return system::err::invalid_state;
    }

    if (this->busy_) {
        sys_debug_print(SYSTEM_LEVEL_WARNING, "http::ranged::basic_connection::connect", "Service is busy");
        return system::err::invalid_state;
    }
    ...
    this->busy_ = true;
    //sys_debug_print(SYSTEM_LEVEL_DEBUG, "http::ranged::basic_connection::connect", "Trying to async resolve");
...
void basic_connection::on_resolve(boost::system::error_code ec, boost::asio::ip::tcp::resolver::results_type results, const on_result_f& on_result) {
    ...
    if (ec) {
        sys_debug_print(SYSTEM_LEVEL_ERROR, "http::ranged::basic_connection::on_resolve", "%s (%d)", ec.message().c_str(), ec.value());
        this->busy_ = false;
...
void basic_connection::on_connect(boost::system::error_code ec, const on_result_f& on_result) {
    ...
    if (ec) {
        this->close_stream();
        sys_debug_print(SYSTEM_LEVEL_ERROR, "http::ranged::basic_connection::on_connect", "%s (%d)", ec.message().c_str(), ec.value());
        this->busy_ = false;
        if (on_result) on_result(ec);
        return;
    } else {
...
void basic_connection::on_write_head(boost::system::error_code ec, size_t bytes_transferred, const on_progress_f& on_progress, const on_response_f& on_response) {

    boost::ignore_unused(bytes_transferred);

    if (ec) {
        sys_debug_print(SYSTEM_LEVEL_ERROR, "http::ranged::basic_connection::on_write_head", "%s (%d)", ec.message().c_str(), ec.value());
        this->busy_ = false;
        if (on_response) {
            header_list_t null;
    ...
    //sys_debug_print(SYSTEM_LEVEL_DEBUG, "http::ranged::basic_connection::on_write_head", "Data written, trying to set read timer");
    boost::beast::get_lowest_layer(this->stream()).expires_after(std::chrono::seconds(5));
...
void basic_connection::on_read_head(boost::system::error_code ec, size_t bytes_transferred, const on_progress_f& on_progress, const on_response_f& on_response) {

    boost::ignore_unused(bytes_transferred);

    // Check error
    if (ec) {
        sys_debug_print(SYSTEM_LEVEL_ERROR, "http::ranged::basic_connection::on_read_head", "%s (%d)", ec.message().c_str(), ec.value());
        this->busy_ = false;
        if (on_response) {
            header_list_t null;
            on_response(ec, boost::beast::http::status::ok, null, 0);
    ...
    #ifdef SYSTEM_TRACE_HTTP_REQUESTS
    {
        ...
        for (auto const& it : response)
            stream << it.name() << ": " << it.value().to_string() << std::endl;
        stream << "Total data size: " << bytes_transferred << std::endl;
        sys_debug_print(SYSTEM_LEVEL_TRACE, "http::ranged::basic_connection::on_read_head", "%s", stream.str().c_str());
    }
    #endif // SYSTEM_TRACE_HTTP_REQUESTS
    ...
    if (status != boost::beast::http::status::ok) {
        ...
        if (on_response) {
            ...
            for (auto const& it : response)
                headers[it.name()] = it.value().to_string();
    ...
    if (response.has_content_length()) {
        size_t begin, end;
        size_t length = utils::parse<size_t>(response[boost::beast::http::field::content_length].to_string());
        this->cache_content_length_ = length;
        sys_debug_print(SYSTEM_LEVEL_INFO, "http::ranged::basic_connection::on_read_head", "Content length: %d; Range size: %d", length, this->cache_range_size_);
        ...
        if (!range.empty()) {
            ...
            if (ec) {
                sys_debug_print(SYSTEM_LEVEL_ERROR, "http::ranged::basic_connection::on_read_head", "Failed to get next range");
                this->busy_ = false;
                if (on_response) {
                    header_list_t null;
                    on_response(ec, boost::beast::http::status::ok, null, 0);
        ...
        sys_debug_print(SYSTEM_LEVEL_ERROR, "http::ranged::basic_connection::on_read_head", "No content length found");
        this->busy_ = false;
        if (on_response) {
            header_list_t null;
            on_response(ec, boost::beast::http::status::ok, null, 0);
...
void basic_connection::on_write(boost::system::error_code ec, size_t bytes_transferred, const on_progress_f& on_progress, const on_response_f& on_response) {

    boost::ignore_unused(bytes_transferred);

    if (ec) {
        sys_debug_print(SYSTEM_LEVEL_ERROR, "http::ranged::basic_connection::on_write", "%s (%d)", ec.message().c_str(), ec.value());
        this->busy_ = false;
        if (on_response) {
            header_list_t null;
            on_response(ec, boost::beast::http::status::ok, null, 0);
    ...
    //sys_debug_print(SYSTEM_LEVEL_DEBUG, "http::ranged::basic_connection::on_write", "Data written, trying to set read timer");
    boost::beast::get_lowest_layer(this->stream()).expires_after(std::chrono::seconds(5));
...
void basic_connection::on_read(boost::system::error_code ec, size_t bytes_transferred, const on_progress_f& on_progress, const on_response_f& on_response) {

    boost::ignore_unused(bytes_transferred);

    // Check error
    if (ec) {
        sys_debug_print(SYSTEM_LEVEL_ERROR, "http::ranged::basic_connection::on_read", "%s (%d)", ec.message().c_str(), ec.value());
            ...
            boost::asio::post(
                ...
                [this, on_progress, on_response] (void) -> void {
                    system::error_code ec = this->connect(this->host_, this->port_, [this, on_progress, on_response] (system::error_code ec) -> void {
                        if (ec) {
                            sys_debug_print(SYSTEM_LEVEL_ERROR, "http::ranged::basic_connection::on_read::on_reconnect", "%s (%d)", ec.message().c_str(), ec.value());
                            this->busy_ = false;
                            if (on_response) {
                                header_list_t null;
                                on_response(ec, boost::beast::http::status::ok, null, 0);
                    ...
                    if (ec) {
                        sys_debug_print(SYSTEM_LEVEL_ERROR, "http::ranged::basic_connection::on_read::reconnect", "%s (%d)", ec.message().c_str(), ec.value());
                        this->busy_ = false;
                        if (on_response) {
                            header_list_t null;
                            on_response(ec, boost::beast::http::status::ok, null, 0);
    ...
    if (it != this->response_.end()) {
        ...
        if (this->response_.body().size > this->max_response_size_) {
            sys_debug_print(SYSTEM_LEVEL_FATAL, "http::ranged::basic_connection::on_read", "Segmentation fault");
            this->busy_ = false;
            if (on_response) {
                header_list_t null;
        ...
        std::string range = it->value().to_string();
        range = get_next_range(this->cache_content_length_, this->cache_range_size_, range, begin, end);
        #ifdef SYSTEM_TRACE_HTTP_REQUESTS
        sys_debug_print(SYSTEM_LEVEL_INFO, "http::ranged::basic_connection::on_read", "Read complete. Begin: %d; End: %d", begin, end);
        #endif // SYSTEM_TRACE_HTTP_REQUESTS
        if (on_progress) on_progress(end + 1, this->cache_content_length_);
        if (range.empty()) {
            this->close_file();
            this->busy_ = false;
            sys_debug_print(SYSTEM_LEVEL_INFO, "http::ranged::basic_connection::on_read", "Download complete");
            if (on_response) {
                header_list_t headers;
                for (auto const& it : this->response_)
                    headers[it.name()] = it.value().to_string();
            ...
            ec = this->request_range(range, on_progress, on_response);
            if (ec) {
                sys_debug_print(SYSTEM_LEVEL_ERROR, "http::ranged::basic_connection::on_read", "Failed to get next range");
                this->busy_ = false;
                if (on_response) {
                    header_list_t null;
                    on_response(ec, boost::beast::http::status::ok, null, 0);
        ...
        sys_debug_print(SYSTEM_LEVEL_ERROR, "http::ranged::basic_connection::on_read", "No content range found");
        this->busy_ = false;
        if (on_response) {
            header_list_t null;
