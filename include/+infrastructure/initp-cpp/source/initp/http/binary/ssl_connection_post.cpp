...
system::error_code ssl_connection::post(const std::string& target, const header_list_t& head, const std::vector<uint8_t>& body, const on_response_f& on_response) {

    if (!this->connected_) {
        sys_debug_print(SYSTEM_LEVEL_WARNING, "http::binary::ssl_connection::post", "Not connected");
        return system::err::invalid_state;
    }

    if (this->busy_) {
        sys_debug_print(SYSTEM_LEVEL_WARNING, "http::binary::ssl_connection::post", "Service is busy");
        return system::err::invalid_state;
    }
    ...
    #ifdef SYSTEM_TRACE_HTTP_REQUESTS
    {
        ...
        sys_debug_print(SYSTEM_LEVEL_TRACE, "http::binary::ssl_connection::post", "%s", stream.str().c_str());
    }
    #endif // SYSTEM_TRACE_HTTP_REQUESTS

    //sys_debug_print(SYSTEM_LEVEL_DEBUG, "http::binary::ssl_connection::post", "Trying to set write timer");
...
system::error_code ssl_connection::post(const std::string& target, const header_list_t& head, const uint8_t* body, size_t size, const on_response_f& on_response) {
