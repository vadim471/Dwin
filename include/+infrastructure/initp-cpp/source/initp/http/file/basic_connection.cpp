...
system::error_code basic_connection::connect(const std::string& host, const std::string& port, on_result_f on_result) {
    ...
    if (this->connected_) {
        sys_debug_print(SYSTEM_LEVEL_WARNING, "http::file::basic_connection::connect", "Already connected");
        return system::err::invalid_state;
    }

    if (this->busy_) {
        sys_debug_print(SYSTEM_LEVEL_WARNING, "http::file::basic_connection::connect", "Service is busy");
        return system::err::invalid_state;
    }
    ...
    this->busy_ = true;
    //sys_debug_print(SYSTEM_LEVEL_DEBUG, "http::file::basic_connection::connect", "Trying to async resolve");
...
void basic_connection::on_resolve(boost::system::error_code ec, boost::asio::ip::tcp::resolver::results_type results, on_result_f on_result) {
    ...
    if (ec) {
        sys_debug_print(SYSTEM_LEVEL_ERROR, "http::file::basic_connection::on_resolve", "%s (%d)", ec.message().c_str(), ec.value());
...
void basic_connection::on_connect(boost::system::error_code ec, on_result_f on_result) {
    ...
    if (ec) {
        this->close_stream();
        sys_debug_print(SYSTEM_LEVEL_ERROR, "http::file::basic_connection::on_connect", "%s (%d)", ec.message().c_str(), ec.value());
        if (on_result) on_result(ec);
        this->busy_ = false;
        return;
    } else {
...
void basic_connection::on_write(boost::system::error_code ec, std::size_t bytes_transferred, const std::string& filename, on_response_f on_response) {

    boost::ignore_unused(bytes_transferred);

    if (ec) {
        sys_debug_print(SYSTEM_LEVEL_ERROR, "http::file::basic_connection::on_write", "%s (%d)", ec.message().c_str(), ec.value());
        if (on_response) {
            header_list_t null;
...
void basic_connection::on_read(boost::system::error_code ec, std::size_t bytes_transferred, on_response_f on_response) {
    ...
    if (!size && status == boost::beast::http::status::ok) {
        ...
        if (it != this->response_.end()) {
            try {
                size = boost::lexical_cast<size_t>(this->response_[boost::beast::http::field::content_length].to_string());
    ...
    if (ec) {
        sys_debug_print(SYSTEM_LEVEL_ERROR, "http::file::basic_connection::on_read", "%s (%d)", ec.message().c_str(), ec.value());
    } else {
        ...
        #ifdef SYSTEM_TRACE_HTTP_REQUESTS
        {
            ...
            sys_debug_print(SYSTEM_LEVEL_TRACE, "http::file::basic_connection::on_read", "%s", stream.str().c_str());
        }
        #endif // SYSTEM_TRACE_HTTP_REQUESTS
