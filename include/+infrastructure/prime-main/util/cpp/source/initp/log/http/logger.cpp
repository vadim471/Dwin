...
#include <initp/log/http/DataContracts.pb.h>
...
void logger::set_certificate(
    ...
    initp::http::binary::ssl_connection::initialize_ssl(this->ssl_context_, ca, cl, key);
}
...
void logger::initialize(
    ...
    if (delay < 1000)
        delay = 1000;
    this->delay_ = delay;
    this->start_time_ = system::time::now();
...
void logger::start(bool enable) {
    ...
    this->connection_ = std::make_shared<initp::http::binary::ssl_connection>(this->context_, this->ssl_context_);
    ...
    this->thread_.reset(new std::thread([this] (void) -> void {
        ...
        sys_debug_print(SYSTEM_LEVEL_INFO, "log::http::logger::start", ">>> Logger thread started");
        this->context_.run(ec);
        sys_debug_print(SYSTEM_LEVEL_INFO, "log::http::logger::start", "<<< Logger thread stopped");
    }));
}
...
void logger::flush(void) {
    ...
    if (this->flush_) {
        this->flush_ = false;
        sys_debug_print(SYSTEM_LEVEL_ERROR, "log::http::logger::flush", "Failed to flush logger");
    }
}
...
void logger::push_message(
    ...
    if (this->queue_.size() >= this->max_batch_size_ &&
        ...
        if (ec) {
            sys_debug_print(SYSTEM_LEVEL_ERROR, "log::http::logger::push_message", "Failed to connect to %s", this->remote_host_.c_str());
        }
    }
}
...
void logger::batch_messages(void) {
    ...
    for (size_t i = 0; i < this->max_batch_size_ && !this->queue_.empty(); ++i) {
        ...
        if (!item) {
            sys_debug_print(SYSTEM_LEVEL_ERROR, "log::http::logger::batch_messages", "Failed to add item to protobuf container");
            break;
        }
    ...
    if (batch.items_size() > 0) {
        ...
        if (!batch.SerializeToArray(&this->data_to_send_[0], size)) {
            sys_debug_print(SYSTEM_LEVEL_ERROR, "log::http::logger::batch_messages", "Failed to serialize protobuf");
            return;
        }
    }
}
...
void logger::send_messages(void) {
    ...
    if (this->connection_->busy()) {
        sys_debug_print(SYSTEM_LEVEL_ERROR, "log::http::logger::send_messages", "Requested message send, but connection are busy");
        return;
    }
    ...
    if (ec) {
        sys_debug_print(SYSTEM_LEVEL_ERROR, "log::http::logger::send_messages", "Failed to post messages batch");
        this->timer_.expires_after(std::chrono::milliseconds(this->delay_));
        this->connection_->close();
    }
}
...
void logger::do_flush(void) {
    ...
    if (!this->data_to_send_.empty() || !this->queue_.empty()) {
        if (!this->connection_->connected()) {
            system::error_code ec = this->connection_->connect(this->remote_host_, this->remote_port_, RBIND(on_connect, _1));
            if (ec) {
                sys_debug_print(SYSTEM_LEVEL_ERROR, "log::http::logger::do_flush", "Failed to connect to %s", this->remote_host_.c_str());
            }
        }
    } else {
        this->flush_ = false;
...
void logger::on_response(boost::system::error_code ec, boost::beast::http::status status, const uint8_t* body, size_t size) {
    ...
    this->timer_.expires_after(std::chrono::milliseconds(this->delay_));
    if (ec) {
        sys_debug_print(SYSTEM_LEVEL_ERROR, "initp::log::http::logger::on_response", "Failed to send data or get response");
        this->connection_->close();
    } else if (status != boost::beast::http::status::ok) {
        sys_debug_print(SYSTEM_LEVEL_ERROR, "initp::log::http::logger::on_response", "Server returned status %d", (int)status);
        this->connection_->close();
    } else {
        if (!body) {
            sys_debug_print(SYSTEM_LEVEL_FATAL, "log::http::logger::on_response", "No error, but data is null");
            this->connection_->close();
            return;
        }
        ...
        if (size) {
            if (!result.ParseFromArray(body, size)) {
                sys_debug_print(SYSTEM_LEVEL_ERROR, "log::http::logger::on_response", "Failed to parse response protobuf");
                this->connection_->close();
            } else {
                if (!result.apiresultstatus()) {
                    sys_debug_print(SYSTEM_LEVEL_ERROR, "log::http::logger::on_response", "Server respond with failed status");
                    this->connection_->close();
                } else {
                    //sys_debug_print(SYSTEM_LEVEL_TRACE, "log::http::logger::on_response", "Messages was sent");
            ...
            sys_debug_print(SYSTEM_LEVEL_ERROR, "log::http::logger::on_response", "Response body size is null");
            this->connection_->close();
        }
    }
}
...
void logger::on_timer(boost::system::error_code ec) {
    ...
    if (ec == boost::system::errc::operation_canceled) {
        ...
        sys_debug_print(SYSTEM_LEVEL_INFO, "log::http::logger::on_timer", "%s (%d)", ec.message().c_str(), ec.value());
        return;
    }
    ...
    if (this->timer_.expiry() <= boost::asio::steady_timer::clock_type::now()) {
        if (!this->connection_->connected() &&
            ...
            system::error_code ec = this->connection_->connect(this->remote_host_, this->remote_port_, RBIND(on_connect, _1));
            if (ec) {
                sys_debug_print(SYSTEM_LEVEL_ERROR, "log::http::logger::on_timer", "Failed to connect to %s", this->remote_host_.c_str());
            }
        }
    }
...
int64_t logger::to_local_time(system::time_t time) {
    int64_t value(time);
    int64_t timezone = tools::date::timezone_milliseconds();
    if (timezone < 0) {
        if (value > -timezone) {
            value += timezone;
        } else value = 0;
