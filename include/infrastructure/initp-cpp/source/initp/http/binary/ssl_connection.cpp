...
system::error_code ssl_connection::initialize_ssl(boost::asio::ssl::context& ssl, const std::string& cacert, const std::string& clcert, const std::string& key) {
    ...
    if (!cacert.empty() && !clcert.empty() && !key.empty()) {
        ...
        if (ec) {
            ...
            sys_debug_print(SYSTEM_LEVEL_ERROR, "http::binary::ssl_connection::initialize_ssl", "Failed to initialize SSL context. %s (%d)", ec.message().c_str(), ec.value());
            return ec;
        }
    } else {
        ssl.set_verify_mode(boost::asio::ssl::verify_none, ec);
        if (ec) {
            sys_debug_print(SYSTEM_LEVEL_ERROR, "http::binary::ssl_connection::initialize_ssl", "Failed to set no verify mode, %s (%d)", ec.message().c_str(), ec.value());
            return ec;
        }
    }
...
system::error_code ssl_connection::connect(const std::string& host, const std::string& port, const on_result_f& on_result) {
    ...
    if (this->connected_) {
        sys_debug_print(SYSTEM_LEVEL_WARNING, "http::binary::ssl_connection::connect", "Already connected");
        return system::err::invalid_state;
    }

    if (this->busy_) {
        sys_debug_print(SYSTEM_LEVEL_WARNING, "http::binary::ssl_connection::connect", "Service is busy");
        return system::err::invalid_state;
    }
    ...
    if (!SSL_set_tlsext_host_name(this->stream_->native_handle(), host.c_str())) {
        ...
        sys_debug_print(SYSTEM_LEVEL_ERROR, "http::binary::ssl_connection::connect", "%s (%d)", ec.message().c_str(), ec.value());
        return system::err::internal_error;
    }

    this->busy_ = true;
    //sys_debug_print(SYSTEM_LEVEL_DEBUG, "http::binary::ssl_connection::connect", "Trying to async resolve");
...
system::error_code ssl_connection::disconnect(const on_result_f& on_result) {
    ...
    if (!this->connected_) {
        sys_debug_print(SYSTEM_LEVEL_WARNING, "http::binary::ssl_connection::disconnect", "Not connected");
        return system::err::invalid_state;
    }

    if (this->busy_) {
        sys_debug_print(SYSTEM_LEVEL_WARNING, "http::binary::ssl_connection::disconnect", "Service is busy");
        return system::err::invalid_state;
    }
...
void ssl_connection::on_resolve(boost::system::error_code ec, boost::asio::ip::tcp::resolver::results_type results, const on_result_f& on_result) {
    ...
    if (ec) {
        sys_debug_print(SYSTEM_LEVEL_ERROR, "http::binary::ssl_connection::on_resolve", "%s (%d)", ec.message().c_str(), ec.value());
        this->busy_ = false;
    ...
    //sys_debug_print(SYSTEM_LEVEL_DEBUG, "http::binary::ssl_connection::on_resolve", "Trying to set connection timer");
...
void ssl_connection::on_connect(boost::system::error_code ec, const on_result_f& on_result) {
    ...
    if (ec) {
        this->close_stream();
        sys_debug_print(SYSTEM_LEVEL_ERROR, "http::binary::ssl_connection::on_connect", "%s (%d)", ec.message().c_str(), ec.value());
        this->busy_ = false;
        if (on_result) on_result(ec);
        return;
    }

    //sys_debug_print(SYSTEM_LEVEL_DEBUG, "http::binary::ssl_connection::on_connect", "Connected, call handshake");
...
void ssl_connection::on_handshake(boost::system::error_code ec, const on_result_f& on_result) {
    ...
    if (ec) {
        sys_debug_print(SYSTEM_LEVEL_ERROR, "http::binary::ssl_connection::on_handshake", "%s (%d)", ec.message().c_str(), ec.value());
        this->close_stream();
    } else {
...
void ssl_connection::on_write(boost::system::error_code ec, std::size_t bytes_transferred, const on_response_f& on_response) {

    boost::ignore_unused(bytes_transferred);

    if (ec) {
        sys_debug_print(SYSTEM_LEVEL_ERROR, "http::binary::ssl_connection::on_write", "%s (%d)", ec.message().c_str(), ec.value());
        this->busy_ = false;
        if (on_response) {
            header_list_t null;
    ...
    //sys_debug_print(SYSTEM_LEVEL_DEBUG, "http::binary::ssl_connection::on_write", "Data written, trying to set read timer");
    boost::beast::get_lowest_layer(this->stream()).expires_after(std::chrono::seconds(5));
...
void ssl_connection::on_read(boost::system::error_code ec, std::size_t bytes_transferred, const on_response_f& on_response) {
    ...
    if (ec) {
        sys_debug_print(SYSTEM_LEVEL_ERROR, "http::binary::ssl_connection::on_read", "%s (%d)", ec.message().c_str(), ec.value());
    } else {
        ...
        #ifdef SYSTEM_TRACE_HTTP_REQUESTS
        {
            ...
            sys_debug_print(SYSTEM_LEVEL_TRACE, "http::binary::ssl_connection::on_read", "%s", stream.str().c_str());
        }
        #endif // SYSTEM_TRACE_HTTP_REQUESTS

        if (this->response_.body().size > this->max_response_size_) {
            sys_debug_print(SYSTEM_LEVEL_FATAL, "http::binary::ssl_connection::on_read", "Segmentation fault");
            this->busy_ = false;
            if (on_response) {
                header_list_t null;
