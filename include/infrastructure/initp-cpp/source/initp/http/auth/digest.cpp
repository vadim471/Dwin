...
bool generate(
    ...
    nonce = utils::hash::md5(stream.str());
    boost::algorithm::to_lower(nonce);
    stream.str("");
    stream << time << ":http:digest";
    opaque = utils::hash::md5(stream.str());
...
bool check(
    ...
    std::string HA1 = utils::hash::md5(stream.str());
    boost::algorithm::to_lower(HA1);
    stream.str("");
    // A2
    stream << method << ":" << uri;
    std::string HA2 = utils::hash::md5(stream.str());
    boost::algorithm::to_lower(HA2);
    stream.str("");
    // Response
    stream << HA1 << ":" << nonce << ":" << nc << ":" << cnonce << ":" << qop << ":" << HA2;
    std::string result = utils::hash::md5(stream.str());
